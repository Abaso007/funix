...
def get_type_dict(annotation: any) -> dict:
    ...
    if annotation_type_class_name == "_GenericAlias":
        ...
        if getattr(annotation, "_name") in ["List", "Dict"]:
            return {"type": str(annotation)}
        elif (
        ...
        if getattr(annotation, "_name") in ["Dict", "List"]:
            return {"type": str(annotation)}
    elif annotation_type_class_name == "_TypedDictMeta":
        key_and_type = {
            key: (
                supported_basic_types_dict[
                    annotation.__annotations__[key].__name__
                ]
        ...
        optional_config |= get_type_dict(getattr(annotation, "__args__")[0])
...
def get_type_widget_prop(
    ...
    # Basic and List only
    if (
        not isinstance(function_arg_widget, str)
        and isinstance(function_arg_widget, list)
        and index >= len(function_arg_widget)
        or not isinstance(function_arg_widget, str)
        and not isinstance(function_arg_widget, list)
    ):
        widget = ""
    elif not isinstance(function_arg_widget, str):
    ...
    if function_arg_type_name in supported_basic_types:
        ...
        if isinstance(typing_list_search_result, Match):  # typing.List, typing.Dict
            content_type = typing_list_search_result["contentType"]
            ...
            function_arg_type_name == "typing.Dict"
            or function_arg_type_name != "typing.List"
        ):
...
def funix_param_to_widget(annotation: any) -> str:
    ...
    """
    if need_config := hasattr(annotation, "__funix_config__"):
